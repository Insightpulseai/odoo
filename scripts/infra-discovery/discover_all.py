...
SYSTEM_TENANT_ID = os.environ.get('SYSTEM_TENANT_ID', '00000000-0000-0000-0000-000000000001')


class InfraDiscoveryOrchestrator:
    """Orchestrates infrastructure discovery across all sources."""

    def __init__(self, supabase_url: str, supabase_key: str, tenant_id: str, dry_run: bool = False):
    ...
    def start_run(self, discovery_type: str) -> Optional[str]:
        ...
        result = self.client.rpc('kg.start_discovery_run', {
    ...
    def complete_run(self, run_id: str, error: Optional[str] = None):
        ...
        self.client.rpc('kg.complete_discovery_run', {
            'p_run_id': run_id,
    ...
    def upsert_node(self, kind: str, key: str, label: str, attrs: dict) -> Optional[str]:
        ...
        result = self.client.rpc('kg.upsert_node', {
            'p_tenant': self.tenant_id,
            'p_kind': kind,
            'p_key': key,
            'p_label': label,
    ...
    def upsert_edge(
        ...
        if self.dry_run:
            logger.info(f"[DRY-RUN] Would upsert edge: {src_node_id} --{predicate}--> {dst_node_id}")
            return str(uuid4())

        result = self.client.rpc('kg.upsert_edge', {
            'p_tenant': self.tenant_id,
...
def main():
    parser = argparse.ArgumentParser(description='Infrastructure Discovery Orchestrator')
    parser.add_argument(
        '--source', '-s',
        action='append',
    ...
    if not args.dry_run and (not supabase_url or not supabase_key):
        logger.error("SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY required (or use --dry-run)")
        sys.exit(1)
