...
class ProjectTask(models.Model):
    ...
    ipai_template_id = fields.Many2one(
        "ipai.finance.task.template", string="Template", index=True, ondelete="set null"
    )

    ipai_owner_code = fields.Char(string="Owner Code", index=True)
    ipai_owner_role = fields.Selection(
        [("prep", "Preparation"), ("review", "Review"), ("approve", "Approval")],
        index=True,
    )
    ipai_compliance_step = fields.Selection(
        [
            ("prepare", "Prepare"),
            ("review", "Review"),
            ("approve", "Approve/Pay"),
    ...
    ipai_days_to_deadline = fields.Integer(
        compute="_compute_ipai_deadline_metrics", store=True
    )
