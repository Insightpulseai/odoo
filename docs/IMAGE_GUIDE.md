...
**Image Tag:** `ghcr.io/jgtolentino/odoo:latest`
...
docker tag ghcr.io/jgtolentino/odoo:latest \
           ghcr.io/jgtolentino/odoo:sha-$(git rev-parse --short HEAD)
...
docker push ghcr.io/jgtolentino/odoo:sha-$(git rev-parse --short HEAD)
```
...
docker exec -it odoo odoo -d odoo -u ipai_finance_ppm --stop-after-init
...
4. **Image scanning** - Run `trivy image ghcr.io/jgtolentino/odoo:latest` before deploying
