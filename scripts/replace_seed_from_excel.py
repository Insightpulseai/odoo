...
def parse_closing_tasks(df):
    ...
    for idx, row in df.iterrows():
        ...
        if emp_code.lower() == 'nan': emp_code = ""
...
def parse_tax_filing(df):
    ...
    for idx, row in df.iterrows():
        ...
        # Filter Junk
        if not form or form.lower() in ['nan', 'nat', 'none']: continue
        if "bir form" in form.lower(): continue # Header row
        if "step" in form.lower() and "process" in form.lower(): continue # Instruction row
...
def run():
    ...
    with open(os.path.join(out_dir, "tasks.csv"), 'w', newline='') as f:
        fieldnames = ['xid_task', 'xid_project', 'task_name', 'description', 'stage_name', 'user_raw', 'active', 'date_deadline']
    ...
    def get_or_create_xml_id(model, xid, dry_run_mode):
        ...
        # Search
        res = models.execute_kw(db, uid, password, 'ir.model.data', 'search_read', 
    ...
    for xid, pname in projects.items():
        ...
        if rid:
            ...
            if not args.dry_run:
                models.execute_kw(db, uid, password, 'project.project', 'write', [[rid], vals])
        else:
            print(f"CREATE Project {xid}")
            if not args.dry_run:
                rid = models.execute_kw(db, uid, password, 'project.project', 'create', [vals])
    ...
    for r in processed:
        ...
        if tid:
            ...
            if not args.dry_run:
                models.execute_kw(db, uid, password, 'project.task', 'write', [[tid], vals])
        else:
            print(f"CREATE Task {txid}")
            if not args.dry_run:
                tid = models.execute_kw(db, uid, password, 'project.task', 'create', [vals])
