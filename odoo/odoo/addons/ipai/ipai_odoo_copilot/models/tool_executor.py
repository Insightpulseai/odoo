...
class CopilotToolExecutor(models.Model):
    ...
    def execute_tool(self, tool_name, arguments, context_envelope):
        ...
        if settings.get("read_only_mode") and tool_name not in self._READ_ONLY_TOOLS:
            reason = "Tool '%s' blocked: system is in read-only mode" % tool_name
            self._log_tool_audit(
                tool_name, arguments, context_envelope,
    ...
    def _validate_scope(self, model, record_ids, context_envelope):
        ...
        entity_ids = context_envelope.get("entity_ids", [])
    ...
    def _execute_search_docs(self, arguments, context_envelope):
        ...
        return {"results": [], "source": "none", "message": "No search backend configured"}
    ...
    def _search_via_azure_ai(self, query, kb_scope, limit,
        ...
        if kb_scope and kb_scope != "general-kb":
            filter_expr = "scope eq '%s'" % kb_scope
    ...
    def _execute_search_odoo_docs(self, arguments, context_envelope):
        ...
        filter_expr = None
        if module:
            filter_expr = "module eq '%s'" % module
    ...
    def _execute_search_azure_docs(self, arguments, context_envelope):
        ...
        filter_expr = None
        if service:
            filter_expr = "service eq '%s'" % service
    ...
    def _execute_search_databricks_docs(self, arguments, context_envelope):
        ...
        filter_parts = []
        if domain:
            filter_parts.append("domain eq '%s'" % domain)
        if industry:
            filter_parts.append("industry eq '%s'" % industry)
    ...
    def _execute_search_org_docs(self, arguments, context_envelope):
        ...
        filter_parts = []
        if doc_type:
            filter_parts.append("doc_type eq '%s'" % doc_type)
        if source:
            filter_parts.append("source eq '%s'" % source)
        filter_expr = " and ".join(filter_parts) if filter_parts else None
    ...
    def _execute_search_spec_bundles(self, arguments, context_envelope):
        ...
        if bundle:
            filter_parts.append(
                "path ge 'spec/%s' and path lt 'spec/%s~'" % (bundle, bundle)
            )
    ...
    def _execute_view_dashboard(self, arguments, context_envelope):
        ...
        return {
            "available": False,
            "message": "Dashboard module not installed or dashboard not found.",
    ...
    def _log_tool_audit(self, tool_name, arguments, context_envelope,
        ...
        """
        try:
            user_id = context_envelope.get("user_id") or self.env.uid
            ...
            self.env["ipai.copilot.audit"].sudo().create({
                ...
                "response_excerpt": reason[:500] if blocked else "OK",
                "environment_mode": "read_only" if blocked else "read_only",
                "blocked": blocked,
                "reason": (reason or "")[:256],
                "source": source if source in ("discuss", "api", "widget") else "api",
                "surface": surface if surface in ("web", "erp", "copilot", "analytics", "ops") else "copilot",
            })
        except Exception:
