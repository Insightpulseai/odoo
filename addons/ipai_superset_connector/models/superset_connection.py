...
class SupersetConnection(models.Model):
    ...
    auth_method = fields.Selection([
        ...
        ('oauth', 'OAuth2'),
        ('api_key', 'API Key'),
    ], string='Auth Method', default='basic', required=True)
    ...
    def _authenticate(self, session):
        ...
        try:
            ...
            csrf_token = csrf_response.json().get('result') if csrf_response.ok else None
        ...
        try:
            ...
            return {
                ...
                'params': {
                    ...
                    'message': _('Connected as: %s') % result.get('result', {}).get('username', 'Unknown'),
                    'type': 'success',
                    'sticky': False,
                }
            }
    ...
    def action_create_database_connection(self):
        ...
        if not all([self.pg_host, self.pg_database, self.pg_username, self.pg_password]):
        ...
        existing = self._api_call('GET', '/api/v1/database/', params={
            'q': json.dumps({'filters': [{'col': 'database_name', 'opr': 'eq', 'value': self.db_connection_name}]})
        })
        
        if existing.get('count', 0) > 0:
            db_id = existing['result'][0]['id']
            # Update existing
            self._api_call('PUT', f'/api/v1/database/{db_id}', data={
            ...
            # Create new
            result = self._api_call('POST', '/api/v1/database/', data={
                'database_name': self.db_connection_name,
                'sqlalchemy_uri': sqlalchemy_uri,
                'expose_in_sqllab': True,
                'allow_ctas': False,
    ...
    def create_dataset(self, table_name, schema='public', database_id=None):
        ...
        return self._api_call('POST', '/api/v1/dataset/', data={
    ...
    def get_tables(self, schema='public', database_id=None):
        ...
        db_id = database_id or self.db_connection_id
        return self._api_call('GET', f'/api/v1/database/{db_id}/tables/', params={
            'schema_name': schema
        })
