...
class IpaiGridColumn(models.Model):
    ...
    grid_view_id = fields.Many2one(
        "ipai.grid.view", string="Grid View", required=True, ondelete="cascade"
    )

    # Field binding
    field_name = fields.Char(
        string="Field Name", required=True, help="Technical name of the model field"
    )
    label = fields.Char(
        string="Column Header", help="Display label for the column header"
    )
    ...
    width = fields.Integer(default=150, help="Column width in pixels")
    min_width = fields.Integer(default=80, help="Minimum column width in pixels")
    max_width = fields.Integer(default=500, help="Maximum column width in pixels")
    ...
    # Formatting
    format_string = fields.Char(help="Python format string for displaying values")
    date_format = fields.Char(default="%Y-%m-%d", help="Date format string")
    decimal_places = fields.Integer(
        default=2, help="Number of decimal places for numeric fields"
    )
    ...
    # Interactivity
    sortable = fields.Boolean(default=True, help="Allow sorting by this column")
    filterable = fields.Boolean(default=True, help="Allow filtering by this column")
    searchable = fields.Boolean(default=True, help="Include in quick search")
    resizable = fields.Boolean(default=True, help="Allow resizing this column")
    editable = fields.Boolean(default=False, help="Allow inline editing")
    clickable = fields.Boolean(
        default=True, help="Make cell content clickable (opens record)"
    )
    ...
    widget_options = fields.Text(
        default="{}", help="Widget-specific configuration as JSON"
    )

    # Styling
    css_class = fields.Char(help="Additional CSS classes for the column")
    header_css_class = fields.Char(help="CSS classes for the column header")
    cell_css_class = fields.Char(help="CSS classes for cells in this column")

    # Special columns
    is_primary = fields.Boolean(
        default=False, help="Primary display column (usually the name field)"
    )
    is_avatar_column = fields.Boolean(
        default=False, help="Column displays user/record avatar"
    )
    is_selection_column = fields.Boolean(
        default=False, help="This is the row selection checkbox column"
    )
    is_action_column = fields.Boolean(
        default=False, help="This column contains row action buttons"
    )
    ...
    def _compute_display_name(self):
        for record in self:
            record.display_name = (
                record.label or record.field_name or _("Unnamed Column")
            )
