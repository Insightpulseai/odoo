...
def discover_supabase(orchestrator) -> Dict[str, Any]:
    ...
    """
    supabase_url = os.environ.get('SUPABASE_URL', 'https://spdtwktxdalcfigzeqrz.supabase.co')
    ...
    try:
        ...
        known_schemas = ['public', 'kb', 'kg', 'agent_mem', 'docs', 'ops', 'ops_advisor', 'odoo_shadow', 'auth', 'storage']
        ...
        for schema_name in known_schemas:
            schema_node_id = orchestrator.upsert_node(
                ...
                attrs={
                    'project_ref': project_ref,
                    'schema_type': 'api_exposed' if schema_name in ['public', 'kb', 'kg', 'agent_mem', 'docs', 'ops_advisor'] else 'internal'
                }
            )
        ...
        known_tables = {
            'kb': ['spaces', 'artifacts', 'versions', 'blocks', 'citations', 'catalog_sources', 'catalog_nodes', 'harvest_state'],
            'kg': ['nodes', 'edges', 'evidence', 'node_embeddings', 'predicate_catalog', 'discovery_runs'],
            'agent_mem': ['sessions', 'events', 'skills', 'agent_skill_bindings', 'memory_sync_log'],
        ...
        if access_token:
            ...
            known_functions = [
                'docs-ai-ask', 'semantic-query', 'semantic-import-osi', 'semantic-export-osi',
                'sync-kb-to-schema', 'catalog-sync', 'verify-secrets', 'verify-supabase',
                'verify-github', 'verify-external-apis', 'mailgate-mailgun', 'n8n-proxy'
            ]
