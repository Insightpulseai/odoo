...
class BirFormSchedule(models.Model):
    ...
    def action_send_due_date_9am_reminder(self):
        ...
        _logger.info(f"Sent 9AM reminders for {len(forms_due_today)} BIR forms due today")
        return True
    ...
    def action_send_due_date_5pm_reminder(self):
        ...
        _logger.info(f"Sent 5PM reminders for {len(forms_due_today)} BIR forms due today")
        return True
    ...
    def _send_reminder_webhook(self, form, reminder_type, webhook_url):
        ...
        try:
            ...
            _logger.info(f"Reminder sent for {form.form_code} ({form.period}): {reminder_type}")
    ...
    def _send_overdue_webhook(self, form, days_overdue, penalty_risk, webhook_url):
        ...
        try:
            ...
            response.raise_for_status()
            _logger.info(f"Overdue nudge sent for {form.form_code}: {days_overdue} days overdue")
        except Exception as e:
            _logger.error(f"Failed to send overdue nudge for {form.form_code}: {str(e)}")
