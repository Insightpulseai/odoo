...
def export_todo_seed(env):
    ...
    domain = [('project_id', 'in', project_ids), ('active', 'in', [True, False])] # Include archived? User said "COMPLETE task seed data", implying all.
    ...
    for t in tasks:
        ...
        for u in users: ref_users.add(u)

        tags = t.tag_ids
        task_data['tag_names'] = ",".join([tag.name for tag in tags])
        for tag in tags: ref_tags.add(tag)

        if t.stage_id: ref_stages.add(t.stage_id)
        if t.project_id: ref_projects.add(t.project_id)
    ...
    fieldnames = ['id', 'xml_id', 'name', 'project_id', 'project_name', 'stage_id', 'stage_name', 
                  'active', 'description', 'priority', 'sequence', 'date_deadline', 
                  'parent_id', 'parent_name', 'company_id', 'user_logins', 'tag_names']
    ...
    with open(stages_file, 'w', newline='', encoding='utf-8') as f:
        ...
        for s in ref_stages:
            writer.writerow([s.id, get_xml_id(env, s), s.name, s.sequence, 
    ...
    print("-" * 65)
    print(f"{'TOTAL':<30} | {total_expected:<10} | {total_actual:<10} | {'PASS' if total_expected == total_actual else 'FAIL'}")
