...
class IpaiTenant(models.Model):
    ...
    sequence = fields.Integer(
        string="Sequence", default=10, help="Display order in lists"
    )
    ...
    # Business Info
    company_name = fields.Char(string="Legal Company Name", tracking=True)
    ...
    country_id = fields.Many2one(
        "res.country", string="Country", default=lambda self: self.env.ref("base.ph").id
    )

    # Contact Info
    admin_email = fields.Char(string="Admin Email", tracking=True)
    admin_phone = fields.Char(string="Admin Phone")

    # Metadata
    onboarded_date = fields.Date(
        string="Onboarding Date", default=fields.Date.context_today, tracking=True
    )
    ...
    def _check_schema_format(self):
        ...
        for record in self:
            if record.supabase_schema and not re.match(
                r"^[a-z_][a-z0-9_]*$", record.supabase_schema
            ):
                raise ValidationError(
                    _(
    ...
    def action_provision_supabase_schema(self):
        ...
        return {
            ...
            "params": {
                ...
                "message": _("Supabase schema provisioning for %s initiated.")
                % self.code,
                "type": "info",
                "sticky": False,
            },
    ...
    def action_open_superset_workspace(self):
        ...
        if not self.superset_base_url:
            raise ValidationError(
                _("Superset base URL not configured for this tenant.")
            )
