...
class FinancePPMGoLiveChecklist(models.Model):
    ...
    name = fields.Char(string='Checklist Name', required=True, default='Finance PPM Go-Live')
    ...
    supervisor_review_date = fields.Datetime(string='Supervisor Review Date', readonly=True)

    senior_supervisor_id = fields.Many2one('res.users', string='Senior Supervisor Finance')
    senior_supervisor_review_date = fields.Datetime(string='Senior Supervisor Review Date', readonly=True)
    ...
    director_signoff_date = fields.Datetime(string='Director Sign-Off Date', readonly=True)
    ...
    # Progress tracking
    total_items = fields.Integer(string='Total Items', compute='_compute_progress', store=True)
    completed_items = fields.Integer(string='Completed Items', compute='_compute_progress', store=True)
    completion_pct = fields.Float(string='Completion %', compute='_compute_progress', store=True)
    ...
    def _compute_progress(self):
        for record in self:
            ...
            record.completion_pct = (record.completed_items / record.total_items * 100) if record.total_items > 0 else 0.0
    ...
    def action_reject(self):
        ...
        self.created_by.notify_warning(
            message=f'Finance PPM Go-Live Checklist REJECTED',
