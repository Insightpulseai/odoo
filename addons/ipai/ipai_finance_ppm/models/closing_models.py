...
class CloseTaskTemplate(models.Model):
    ...
    recurrence_rule = fields.Char(
        string="Recurrence Rule", help="RRULE format (e.g., FREQ=MONTHLY;BYMONTHDAY=1)"
    )
    ...
    employee_code = fields.Char(
        string="Default Employee Code", help="Default assignee employee code"
    )

    # Clarity PPM Fields
    wbs_code_template = fields.Char(
        string="WBS Code Template", help="Template for WBS code generation"
    )
...
class CloseGenerationRun(models.Model):
    ...
    dry_run = fields.Boolean(
        string="Dry Run", default=True, help="If True, simulate without creating tasks"
    )
    ...
    report_json = fields.Text(
        string="Generation Report", help="JSON report with counts, warnings, errors"
    )
    ...
    warning_count = fields.Integer(
        string="Warnings", compute="_compute_counts", store=True
    )
    ...
    generated_task_ids = fields.One2many(
        "ipai.close.generated.map", "generation_run_id", string="Generated Tasks"
    )
...
class CloseGeneratedMap(models.Model):
    ...
    template_id = fields.Many2one(
        "ipai.close.task.template", string="Source Template", ondelete="set null"
    )
