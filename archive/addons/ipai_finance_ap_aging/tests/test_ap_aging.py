...
class TestAPAging(TransactionCase):
    ...
    def test_ap_aging_bucket_0_30(self):
        ...
        self.assertGreater(
            float(vendor["bucket_0_30"]), 0, "Should have amount in 0-30 bucket"
        )
    ...
    def test_ap_aging_bucket_31_60(self):
        ...
        vendor = result["vendors"][0]
        self.assertGreater(
            float(vendor["bucket_31_60"]), 0, "Should have amount in 31-60 bucket"
        )
    ...
    def test_ap_aging_bucket_61_90(self):
        ...
        vendor = result["vendors"][0]
        self.assertGreater(
            float(vendor["bucket_61_90"]), 0, "Should have amount in 61-90 bucket"
        )
    ...
    def test_ap_aging_bucket_90_plus(self):
        ...
        vendor = result["vendors"][0]
        self.assertGreater(
            float(vendor["bucket_90_plus"]), 0, "Should have amount in 90+ bucket"
        )
    ...
    def test_employee_context_filtering(self):
        ...
        result_rim = self.env["account.move.line"].cron_generate_ap_aging_snapshot(
            "RIM"
        )

        # Generate snapshot for different employee (should return same data if no employee filter in move lines)
        result_other = self.env["account.move.line"].cron_generate_ap_aging_snapshot(
            "CKVC"
        )
