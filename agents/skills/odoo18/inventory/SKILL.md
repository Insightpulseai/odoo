...
# Inventory — Odoo 18.0 Skill Reference
...
<!-- TODO: not found in docs — Odoo 18 inventory docs do not cover JSON-RPC or XML-RPC examples directly. -->
...
- **Periodic Anglo-Saxon** is now fully supported (was unsupported in 17.0).
...
- In 18.0, `Product Type` is now `Goods`, `Service`, or `Combo`. The old `Consumable` and `Storable Product` distinction has been replaced by the `Track Inventory` checkbox on Goods.
