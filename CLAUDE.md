...
25. **Microsoft Adoption Registry — canonical surface IPAI uses**: This is the locked surface; everything else is OUT until a registry entry justifies it.
    ...
    **Layer 2 — Frameworks + tools (adopt as deps)**:
    ...
    - `microsoft/azure-skills` plugin — operator workflow skills (24 skills, installed v1.0.20 verified 2026-04-21)
    - `databricks-solutions/ai-dev-kit` plugin — Databricks-aware Claude Code skills + MCP (19 skills, installed v1.1.9 verified 2026-04-21). Pairs with azure-skills to cover both planes (Azure infra + Databricks data).
...
31. **Owned-repo taxonomy — 5 post-development lanes, 11 repos in `Insightpulse-ai`**: Codified 2026-04-20 (revised the same day from a too-coarse 3-repo audience model). The org is shaped by **post-development lifecycle stage**, not by coding discipline or audience. The enterprise (`ipai`) is the governance shell (policies, billing, security defaults, organization inventory); the org (`Insightpulse-ai`) is the operating portfolio for what you **package, prove, publish, and operate**.
    ...
    **Web-lane boundary (one repo, holds ALL frontend product surfaces)**: The `web` repo owns user-facing apps + product UX shells. Internal layout (canonical): `web/apps/<product>-pwa/` for browser-installable mobile-web, `web/apps/<product>-ios/` for App Store iPhone wrappers (per Rule 18 native wrapper skill), `web/apps/ops-console/` for internal ops UI (current `/tmp/fluent-demo` Fluent UI v9 scratch migrates here), `web/packages/<name>/` for shared TS (SDK + auth + analytics + ui component layer that consumes `design/` tokens). **Decision rules:** browser-installable mobile-web → PWA in `web/apps/`; App Store iPhone app → iOS in `web/apps/`; shared TS → `web/packages/`; design tokens + Fluent UI patterns → `design/` (NOT `web/`); backend / control-plane / business logic → `platform/` or `agent-platform/` or `odoo/` (NOT `web/`). **Forbidden in `web/`:** backend handlers, MAF runtime imports, agent definitions, Odoo addons, infra Bicep/Terraform, components that belong in `design/`, iOS wrapper code outside `web/apps/`. **Migration mapping:** legacy `Insightpulseai/web/` (Vite Fluent scratch) → `Insightpulse-ai/web/apps/ops-console/`; legacy `Insightpulseai/web/mobile/` (iOS wrapper) → `Insightpulse-ai/web/apps/pulser-ios/`; future PWA → `web/apps/pulser-pwa/`; shared `web/packages/*` paths unchanged.
