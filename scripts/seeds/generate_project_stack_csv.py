...
def generate_partners_csv(data: dict, output_path: Path) -> None:
    ...
    fieldnames = [
        'External ID', 'Name', 'Company Type', 'Is a Company',
        'Parent Company', 'Email', 'Phone', 'Street', 'City', 'Zip',
        'Country', 'Is a Customer', 'Is a Vendor', 'Internal Reference',
        'Job Position', 'Notes'
    ]
...
def generate_products_csv(data: dict, output_path: Path) -> None:
    ...
    fieldnames = [
        'External ID', 'Name', 'Internal Reference', 'Product Type',
        'Sales Price', 'Cost', 'Unit of Measure', 'Invoicing Policy',
        'Track Service', 'Active', 'Can be Sold', 'Sales Description'
    ]
...
def generate_projects_csv(data: dict, output_path: Path) -> None:
    ...
    fieldnames = [
        'External ID', 'Name', 'Customer', 'Analytic Account',
        'Allow Timesheets', 'Billable', 'Timesheet Product', 'Privacy',
        'Active', 'Description', 'Start Date', 'End Date', 'Color'
    ]
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for p in projects:
            row = {
                ...
                'Allow Timesheets': 'TRUE' if p.get('allow_timesheets', True) else 'FALSE',
...
def generate_tasks_csv(data: dict, output_path: Path) -> None:
    ...
    fieldnames = [
        'External ID', 'Name', 'Project', 'Customer', 'Stage',
        'Tags', 'Parent Task', 'Depends on', 'Description',
        'Planned Hours', 'Deadline', 'Start Date', 'Priority', 'Sequence'
    ]
...
def generate_timesheets_csv(data: dict, output_path: Path) -> None:
    ...
    fieldnames = [
        'External ID', 'Date', 'Employee', 'Project', 'Task',
        'Description', 'Hours', 'Analytic Account'
    ]
...
def main():
    ...
    generators = [
        ...
        ('20_analytic_accounts.yaml', 'analytic_accounts.csv', generate_analytic_accounts_csv),
