...
def run(dry_run=False):
    ...
    # 3. EXTRACTION LOGIC
    # Target Projects
    target_projects = ["BIR Tax Filing", "Month-End Close Template", "Month-end closing"]
    ids_to_export = []
    project_map = {}
    ...
    if not dry_run:
        ...
        for name in target_projects:
            # Exact match
            ids = models.execute_kw(db, uid, password, 'project.project', 'search', [[['name', '=', name]]])
            if not ids:
                 # ILIKE
                 ids = models.execute_kw(db, uid, password, 'project.project', 'search', [[['name', 'ilike', name]]])
            
            if ids:
                p_data = models.execute_kw(db, uid, password, 'project.project', 'read', [ids], {'fields': ['name']})
                for p in p_data:
    ...
    tasks = []
    task_fields = [
        'name', 'project_id', 'stage_id', 'user_ids', 'tag_ids', 
        'date_deadline', 'description', 'priority', 'sequence', 
        'parent_id', 'company_id', 'active'
    ]
    
    if ids_to_export and not dry_run:
        domain = [['project_id', 'in', ids_to_export], ['active', 'in', [True, False]]]
        tasks = models.execute_kw(db, uid, password, 'project.task', 'search_read', [domain], {'fields': task_fields})
    ...
    for t in tasks:
        ...
        if pid: ref_projects.add((pid, pname))
        
        sid = t['stage_id'][0] if t['stage_id'] else ''
        sname = t['stage_id'][1] if t['stage_id'] else ''
        if sid: ref_stages.add((sid, sname))
        
        # Collect IDs for batch fetch
        for u in t.get('user_ids', []): ref_users.add(u)
        for tag in t.get('tag_ids', []): ref_tags.add(tag)
    ...
    if not dry_run:
        if ref_users:
            udata = models.execute_kw(db, uid, password, 'res.users', 'read', [list(ref_users)], {'fields': ['name', 'login']})
            for u in udata: user_map[u['id']] = u['login']
            
        if ref_tags:
            tdata = models.execute_kw(db, uid, password, 'project.tags', 'read', [list(ref_tags)], {'fields': ['name', 'color']})
            for tag in tdata: tag_map[tag['id']] = tag['name']
    ...
    # Tasks
    task_headers = ['id', 'name', 'project_id', 'project_name', 'stage_id', 'stage_name', 
                    'active', 'priority', 'sequence', 'date_deadline', 'parent_id', 'parent_name',
                    'company_id', 'description', 'user_logins', 'tag_names']
    ...
    for r in final_rows:
        p = r['project_name']
        if p not in counts: counts[p] = {'total': 0, 'active': 0}
        counts[p]['total'] += 1
        if r['active']: counts[p]['active'] += 1
