...
class IpaiAiRetrievalService(models.AbstractModel):
    ...
    @api.model
    def retrieve_context(self, provider, query, top_k=None):
        """Retrieve relevant context for a query from provider's sources.

        Uses cosine similarity with stable tie-breaking (score DESC, chunk_id ASC).

        Args:
            provider: ipai.ai.provider record (or any record with source_ids)
            query: Query text
        ...
        # Get all chunks from provider's sources
        sources = provider.source_ids.filtered(
