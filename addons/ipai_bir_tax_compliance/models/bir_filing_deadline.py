    ...
    def _cron_generate_deadlines(self):
        ...
        for company in companies:
            ...
            for month in range(1, 13):
                period_end = fields.Date.today().replace(year=current_year, month=month, day=1)
            ...
            for quarter in [3, 6, 9, 12]:
                period_end = fields.Date.today().replace(year=current_year, month=quarter, day=1)
                period_end = period_end + relativedelta(months=1, days=-1)
            ...
            annual_deadline = fields.Date.today().replace(year=current_year + 1, month=4, day=15)
        ...
        for deadline in upcoming:
            ...
            for user in finance_users:
                self.env["mail.activity"].create({
                    "res_model_id": self.env["ir.model"]._get("bir.filing.deadline").id,
                    "res_id": deadline.id,
