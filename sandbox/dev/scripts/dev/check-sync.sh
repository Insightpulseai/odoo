...
if [[ "$HEAD_SHA" == "$ORIGIN_SHA" ]] && \
   ...
   [[ "$DOCKER_COMPOSE" == "✓" ]]; then
    ...
    echo "  - Work in Codespaces: gh codespace create --repo Insightpulseai/odoo --branch main"
else
...
echo "  cd ~/Documents/GitHub/Insightpulseai/odoo/sandbox/dev"
echo "  ./scripts/dev/sync-env.sh"
echo "  ./scripts/dev/start-dev.sh"
echo ""
echo "Daily Start (Codespaces):"
echo "  gh codespace create --repo Insightpulseai/odoo --branch main"
echo "  cd /workspaces/odoo/sandbox/dev"
echo "  ./scripts/dev/sync-env.sh"
echo "  ./scripts/dev/start-dev.sh"
echo ""
