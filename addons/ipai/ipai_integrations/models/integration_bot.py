...
class IntegrationBot(models.Model):
    ...
    # Permissions
    permission_post = fields.Boolean(default=True, help="Can post messages to channels")
    permission_read = fields.Boolean(default=True, help="Can read channel messages")
    permission_manage = fields.Boolean(
        default=False, help="Can manage channels and users"
    )

    # Slash commands
    slash_command_ids = fields.One2many(
        "ipai.integration.bot.command", "bot_id", string="Slash Commands"
    )
...
class IntegrationBotCommand(models.Model):
    ...
    bot_id = fields.Many2one("ipai.integration.bot", required=True, ondelete="cascade")
    command = fields.Char(required=True, help="Command trigger (e.g., /task, /invoice)")
    description = fields.Char(help="Description shown in autocomplete")
    hint = fields.Char(help="Usage hint (e.g., '[task name]')")

    # Handler
    handler_model = fields.Char(help="Odoo model to handle this command")
    handler_method = fields.Char(
        default="_handle_slash_command", help="Method to call on the handler model"
    )
