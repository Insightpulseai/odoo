...
class LogFrameMapper:
    ...
    def _categorize_im1_activity(self, title_lower: str, category: str) -> str:
        ...
        if "bank" in title_lower or "cash" in title_lower:
            ...
            "ar" in title_lower
            or "receivable" in title_lower
            or "ap" in title_lower
            or "payable" in title_lower
        ):
            return "AR/AP aging reviews"
        elif "inventory" in title_lower:
            return "Inventory valuation"
        elif (
            "accrue" in title_lower
            or "accrual" in title_lower
            or "provision" in title_lower
        ):
            ...
            "reclassification" in title_lower
            or "oop" in title_lower
            or "wip" in title_lower
        ):
    ...
    def assign_employee(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(
            ...
            for kw in ["final review", "approval", "cfo", "director"]
        ):
            assigned_to = "CKVC"
        ...
        # Revenue & billable tasks → LAS (Finance Manager)
            ...
            or "revenue" in activity.lower()
            or "billable" in activity.lower()
        ):
            assigned_to = "LAS"
        ...
        # Accruals & Provisions → RIM (complex) or LAS (routine)
            ...
            if any(
                kw in task["title"].lower()
                for kw in ["management fee", "royalty", "consulting"]
            ):
        ...
        task["assigned_to"] = {"employee_code": assigned_to, "role": role}
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Input JSON file with deduplicated tasks",
    )
    parser.add_argument(
        "--output",
        type=Path,
        required=True,
        help="Output JSON file with LogFrame mappings",
    )
