...
def discover_digitalocean(orchestrator) -> Dict[str, Any]:
    ...
    discovered = {"droplets": 0, "databases": 0, "domains": 0, "volumes": 0}
    ...
    try:
        ...
        for domain in domains:
            domain_node_id = orchestrator.upsert_node(
                ...
                label=domain["name"],
                attrs={"ttl": domain.get("ttl"), "zone_file": domain.get("zone_file")},
            )
            discovered["domains"] += 1

            # Get domain records
            records_resp = requests.get(
                f'{DO_API_URL}/domains/{domain["name"]}/records', headers=headers
            )
