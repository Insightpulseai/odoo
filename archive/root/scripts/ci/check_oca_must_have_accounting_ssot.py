#!/usr/bin/env python3
"""
check_oca_must_have_accounting_ssot.py — Validate OCA must-have accounting SSOT registry.
...
SSOT_REL_PATH = "ssot/oca/must_have_accounting.yaml"
SCHEMA_EXPECTED = "ssot.oca.must_have_accounting.v1"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate OCA must-have accounting SSOT registry"
    )
    ...
    if errors:
        if not args.quiet:
            print(
                f"FAIL: oca-must-have-accounting-ssot validation failed ({len(errors)} errors):"
            )
            for err in errors:
                print(f"  {err}")
        sys.exit(3)
    ...
    if not args.quiet:
        print(
            f"PASS: oca-must-have-accounting-ssot validated ({n} modules) — {SSOT_REL_PATH}"
        )
    return 0
