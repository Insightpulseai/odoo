...
echo "---------------------------------------------------"
ssh root@erp.insightpulseai.com "echo '✅ SSH connection successful'" || echo "❌ SSH connection failed"
echo ""

echo "## 2. Odoo Service Status"
echo "---------------------------------------------------"
ssh root@erp.insightpulseai.com "docker ps | grep odoo" || echo "❌ Odoo containers not running"
echo ""

echo "## 3. Installed Modules Count by License"
echo "---------------------------------------------------"
ssh root@erp.insightpulseai.com "docker exec odoo-db-1 psql -U odoo -d odoo -c \
...
echo "---------------------------------------------------"
curl -sf https://ocr.insightpulseai.com/health | jq . 2>/dev/null || echo "❌ OCR adapter health check failed"
echo ""
...
echo "---------------------------------------------------"
ssh root@erp.insightpulseai.com "docker logs odoo-odoo-1 2>&1 | grep 'Odoo version' | tail -1" || echo "❌ Version check failed"
echo ""

echo "## 12. Worker Status"
echo "---------------------------------------------------"
ssh root@erp.insightpulseai.com "docker logs odoo-odoo-1 2>&1 | grep 'Worker.*alive' | tail -10" || echo "❌ Worker status check failed"
echo ""
