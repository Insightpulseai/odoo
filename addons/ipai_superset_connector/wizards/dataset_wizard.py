...
class SupersetDatasetWizard(models.TransientModel):
    ...
    def _default_connection(self):
        ...
        param = (
            ...
            .sudo()
            .get_param("ipai_superset_connector.default_connection_id")
        )
        if param:
            return int(param)
    ...
    def action_create_dataset(self):
        ...
        if existing:
            raise UserError(
                _('Dataset with technical name "%s" already exists')
                % self.technical_name
            )
        ...
        dataset = self.env["superset.dataset"].create(
            {
                ...
                "field_ids": (
                    [(6, 0, self.field_ids.ids)] if not self.include_all_fields else []
                ),
...
class SupersetBulkDatasetWizard(models.TransientModel):
    ...
    preset = fields.Selection(
        [
            ...
            (
                "finance",
                "Finance Suite (account.move, account.move.line, account.account)",
            ),
