...
class BirScheduleLine(models.Model):
    ...
    period_label = fields.Char(required=True, help="e.g., 'Jan 2026' or 'Q1 2026' or 'FY 2025'")
    ...
    @api.constrains("deadline_date", "prep_due_date", "review_due_date", "approve_due_date")
        ...
        for rec in self:
            for d in [rec.prep_due_date, rec.review_due_date, rec.approve_due_date]:
                if d and rec.deadline_date and d > rec.deadline_date:
                    raise ValidationError(_("Step dates must not be after the filing deadline."))
    ...
    def _upsert_from_seed(self, row: dict, strict: bool = False):
        ...
        if not bir_form or not period_label:
            if strict:
                raise ValidationError(_("Seed BIR schedule row missing bir_form/period_label."))
            return False
        vals = {
        ...
        rec = self.search([("bir_form", "=", bir_form), ("period_label", "=", period_label)], limit=1)
        if rec:
            rec.write(vals)
            return rec
