...
class ProjectProject(models.Model):
    ...
    ppm_portfolio_id = fields.Many2one("ppm.portfolio", compute="_compute_ppm_links", store=False)
    ppm_program_id = fields.Many2one("ppm.program", compute="_compute_ppm_links", store=False)
    ...
    def _compute_ppm_links(self):
        for p in self:
            meta = self.env["ppm.project.meta"].search([("project_id", "=", p.id)], limit=1)
