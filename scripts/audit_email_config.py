...
def audit_email(env):
    ...
    if not outgoing_servers:
        ...
        for srv in outgoing_servers:
            print(f"RECORD: ID={srv.id} Host={srv.smtp_host} Port={srv.smtp_port} User={srv.smtp_user} Encryption={srv.smtp_encryption} Active={srv.active}")
    ...
    if not incoming_servers:
        ...
        for srv in incoming_servers:
            ...
            is_ssl = srv.is_ssl
            print(f"RECORD: ID={srv.id} Host={srv.server} Port={srv.port} Type={server_type} SSL={is_ssl} User={srv.user} Active={srv.active}")
    ...
    keys = ['google_gmail_client_id', 'google_gmail_client_secret', 'google_redirect_uri']
    for k in keys:
