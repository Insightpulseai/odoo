        ...
        try:
            ...
            if existing_parent and not any(suffix in existing_parent.template_code for suffix in STEP_SUFFIXES):
                ...
                parent_vals = {
                    ...
                    'task_name_template': reference_tmpl.task_name_template.replace('|PREP', '').replace('|REVIEW', '').replace('|APPROVAL', ''),
                ...
                if hasattr(reference_tmpl, 'description_template'):
                    parent_vals['description_template'] = reference_tmpl.description_template or ''

                if not dry_run:
                    parent_template = TemplateModel.create(parent_vals)
                    print(f"   ✅ Created parent template (ID: {parent_template.id if parent_template else 'N/A'})")
            ...
            for step_tmpl in step_templates:
                ...
                if not step_code:
                    print(f"   ⚠️  Could not determine step code for {step_tmpl.template_code}")
                    continue
                ...
                if not dry_run and parent_template:
                    ...
                    if existing_step:
                        ...
                        step_vals = {
                            ...
                            'sequence': {'PREP': 10, 'REVIEW': 20, 'APPROVAL': 30}.get(step_code, 99),
                            'template_id': parent_template.id,
            ...
            print(f"   ✅ Migrated {base_code} ({len(step_templates)} legacy → 1 parent + {len(step_map)} steps)")
...
Examples:
  ...
  odoo shell -d production < migrate_templates_v1_2_0.py
    ...
    parser.add_argument('--db', default='production', help='Database name (default: production)')
