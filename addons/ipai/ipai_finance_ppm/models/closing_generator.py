...
class ClosingGenerator(models.Model):
    ...
    name = fields.Char(
        string="Generator Name", default="Finance Closing Task Generator"
    )

    @api.model
    def generate_tasks_from_seed(
        self,
        seed_json_path=None,
        seed_json_dict=None,
        cycle_key=None,
        ...
        try:
            ...
            if seed_json_dict:
                ...
                raise ValidationError(
                    "Either seed_json_path or seed_json_dict must be provided"
                )
            ...
            cycle_config = next(
                (
                    c
                    for c in seed_data.get("cycles", [])
                    if c.get("cycle_code") == cycle_type
                ),
                None,
            )
            ...
            if not cycle_config:
                raise ValidationError(
                    f"Cycle type '{cycle_type}' not found in seed JSON"
                )
            ...
            generation_run.write(
                {
                    "status": "failed",
                    "end_time": fields.Datetime.now(),
                    "report_json": json.dumps({"status": "FAIL", "errors": [str(e)]}),
                }
            )
        ...
        for cycle in cycles:
            ...
            for phase in cycle.get("phases", []):
                if not phase.get("phase_code"):
                    errors.append(
                        f"Phase missing phase_code in cycle {cycle.get('cycle_code')}"
                    )
                ...
                for workstream in phase.get("workstreams", []):
                    if not workstream.get("workstream_code"):
                        errors.append(
                            f"Workstream missing workstream_code in phase {phase.get('phase_code')}"
                        )
                    ...
                    for task_template in workstream.get("task_templates", []):
                        if not task_template.get("template_code"):
                            errors.append(
                                f"Task template missing template_code in workstream {workstream.get('workstream_code')}"
                            )
                        if not task_template.get("task_name_template"):
                            errors.append(
                                f"Task template {task_template.get('template_code')} missing task_name_template"
                            )
                        ...
                        for step in task_template.get("steps", []):
                            if not step.get("step_code"):
                                errors.append(
                                    f"Step missing step_code in template {task_template.get('template_code')}"
                                )
        ...
        for cycle in seed_data.get("cycles", []):
            ...
            for phase in cycle.get("phases", []):
                ...
                for workstream in phase.get("workstreams", []):
                    ...
                    workstream_name = workstream.get(
                        "name", workstream.get("workstream_name")
                    )
                    ...
                    for category in workstream.get("categories", []):
                        ...
                        for task_template in category.get("task_templates", []):
                            ...
                            for step in task_template.get("steps", []):
                                ...
                                template_vals = {
                                    "template_code": full_template_code,
                                    "template_version": seed_data.get(
                                        "version", "1.0.0"
                                    ),
                                    ...
                                    "step_code": step_code,
                                    "step_seq": step_seq,
                                    "task_name_template": step.get(
                                        "name_template",
                                        task_template.get("task_name_template"),
                                    ),
                                    "task_description": task_template.get(
                                        "description"
                                    ),
                                    "recurrence_rule": cycle.get("recurrence_rule"),
                                    "duration_days": step.get("duration_days", 1),
                                    "offset_from_period_end": step.get("due", {}).get(
                                        "offset_business_days", 0
                                    ),
                                    "responsible_role": step.get("responsible_role"),
                                    "employee_code": step.get("employee_code"),
                                    "wbs_code_template": task_template.get(
                                        "wbs_code_template"
                                    ),
                                    "critical_path": task_template.get(
                                        "critical_path", False
                                    ),
    ...
    def _generate_task_instances(
        ...
        templates = template_model.search(
            [("cycle_code", "=", cycle_type), ("is_active", "=", True)]
        )
        ...
        for template in templates:
            ...
            template_code_base = (
                template.template_code.split("|")[0]
                if "|" in template.template_code
                else template.template_code
            )
            ...
            task_name = self._render_task_name(
                template.task_name_template, period_start, period_end
            )

            # Compute deadline
            deadline = self._compute_deadline(
                period_end, template.offset_from_period_end
            )
    ...
    def _resolve_assignee(self, employee_code):
        ...
        user = self.env["res.users"].search(
            [("x_employee_code", "=", employee_code)], limit=1
        )
