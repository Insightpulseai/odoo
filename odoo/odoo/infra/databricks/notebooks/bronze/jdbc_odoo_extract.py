...
# Configuration
BRONZE_PATH = spark.conf.get("bronze_storage_path", "abfss://bronze@stipaidev.dfs.core.windows.net")
WATERMARK_PATH = f"{BRONZE_PATH}/odoo_finance/_watermarks"
SECRET_SCOPE = "odoo-pg"

JDBC_URL = dbutils.secrets.get(scope=SECRET_SCOPE, key="jdbc_url")
JDBC_USER = dbutils.secrets.get(scope=SECRET_SCOPE, key="jdbc_user")
JDBC_PASSWORD = dbutils.secrets.get(scope=SECRET_SCOPE, key="jdbc_password")
