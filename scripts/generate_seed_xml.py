...
"""
        ...
        ET.SubElement(project_record, "field", attrib={"name": "name"}).text = (
            "Month-End Closing - Notion Tasks"
        )
        ET.SubElement(project_record, "field", attrib={"name": "active"}).text = "True"
        ET.SubElement(
            project_record, "field", attrib={"name": "privacy_visibility"}
        ).text = "employees"
        ...
        for i, task in enumerate(tasks, 1):
            ...
            record = ET.SubElement(
                data, "record", attrib={"id": task_id, "model": "project.task"}
            )

            # Task name
            ET.SubElement(record, "field", attrib={"name": "name"}).text = (
                self._escape_xml_text(task["title"])
            )
            ...
            if task.get("description"):
                description_field = ET.SubElement(
                    record, "field", attrib={"name": "description"}
                )
            ...
            if user_ref:
                ET.SubElement(
                    record,
                    "field",
                    attrib={"name": "user_ids", "eval": f"[(4, ref('{user_ref}'))]"},
                )
            ...
            if record.find(".//field[@name='description']") is not None:
                ...
                ET.SubElement(record, "field", attrib={"name": "description"}).text = (
                    self._escape_xml_text(logframe_note)
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Odoo XML seed data from Notion tasks"
    )
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
        help="Input JSON file with LogFrame-mapped tasks",
    )
    parser.add_argument(
        "--employees",
        type=Path,
        default=Path("data/employee_directory.json"),
        help="Employee directory JSON",
    )
    parser.add_argument(
        "--output", type=Path, required=True, help="Output XML file path"
    )
