...
| 11 | **Python interpreter pinned to repo venv** — `.devcontainer/devcontainer.json` `customizations.vscode.settings` must set `"python.defaultInterpreterPath": "/workspaces/odoo/.venv/bin/python"` and `"python.terminal.activateEnvironment": true`. This ensures Pylance and Copilot have correct Odoo 19 type context. | Dev Container setup | [Getting Started with Python in VS Code](https://code.visualstudio.com/docs/python/python-tutorial) |
