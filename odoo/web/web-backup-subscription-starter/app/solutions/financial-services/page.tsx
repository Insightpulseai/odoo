...
export default function FinancialServicesSolutionPage() {
  ...
  return (
    <main>
      ...
      {/* Use Cases */}
      {content.useCases && (
        <UseCases title="Real-World Financial Workflows" items={content.useCases} />
      )}

      {/* Resources */}
      {content.resources && (
        <ResourceGrid title="Learn More" items={content.resources} />
      )}

      {/* Partners */}
      {content.partners && (
        <PartnerStrip title={content.partners.title} items={content.partners.items} />
      )}
