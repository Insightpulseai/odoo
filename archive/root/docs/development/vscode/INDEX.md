...
| **Python** | Getting Started with Python in VS Code | <https://code.visualstudio.com/docs/python/python-tutorial> | Covers interpreter selection (`python.defaultInterpreterPath`), venv auto-activation, and Pylance — needed for correct import resolution for Odoo 18 CE source tree and `addons/ipai/` modules inside the Dev Container where the venv lives at `/workspaces/odoo/.venv`. | Set in `.devcontainer/devcontainer.json` `customizations.vscode.settings`: `"python.defaultInterpreterPath": "/workspaces/odoo/.venv/bin/python"` and `"python.terminal.activateEnvironment": true`. | `python.defaultInterpreterPath` |
| **Source Control** | Source control with Git in VS Code | <https://code.visualstudio.com/docs/sourcecontrol/overview> | VS Code maintains its own GitHub credential session (`github.gitAuthentication`) **separate from** `gh` CLI keyring. If VS Code's session expires or uses a different account, it prompts for re-auth and can inject stale credentials into child processes — the same root cause as the `GH_TOKEN=""` 401 pattern. Setting `github.gitAuthentication: false` delegates all auth to `gh` CLI keyring so there is exactly one credential source. Also covers GitHub Pull Requests extension (PR review workflow), Source Control Graph (branch visualisation), and `git.autofetch` for keeping branch status current across 153-workflow CI. | Add to `.devcontainer/devcontainer.json` `customizations.vscode.settings`: `"github.gitAuthentication": false` (delegate to `gh` keyring). Install `GitHub.vscode-pull-request-github` extension in `.vscode/extensions.json`. Enable `git.autofetch: true` for live branch sync in the Dev Container. | `github.gitAuthentication` |
