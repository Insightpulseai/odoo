...
final class MileageTracker: NSObject, CLLocationManagerDelegate {
    ...
    func expenseURL(ratePerKm: Double = 8.0) -> URL {
        ...
        let baseString = Bundle.main.object(forInfoDictionaryKey: "OdooBaseURL") as? String ?? "https://erp.insightpulseai.com/web"
        let route = "#action=expense&distance=\(String(format: "%.1f", distanceKilometers))&amount=\(String(format: "%.2f", amount))"
        return URL(string: "\(baseString)\(route)")!
    }
