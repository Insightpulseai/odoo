...
class Config:
    ...
    master_db_host: str = field(
        default_factory=lambda: os.getenv("MASTER_DB_HOST", "localhost")
    )
    master_db_port: int = field(
        default_factory=lambda: int(os.getenv("MASTER_DB_PORT", "5432"))
    )
    master_db_name: str = field(
        default_factory=lambda: os.getenv("MASTER_DB_NAME", "insightpulse_master")
    )
    master_db_user: str = field(
        default_factory=lambda: os.getenv("MASTER_DB_USER", "postgres")
    )
    master_db_password: str = field(
        default_factory=lambda: os.getenv("MASTER_DB_PASSWORD", "")
    )

    # Template Database
    template_db_name: str = field(
        default_factory=lambda: os.getenv("TEMPLATE_DB_NAME", "insightpulse_template")
    )
    ...
    superset_url: str = field(
        default_factory=lambda: os.getenv("SUPERSET_URL", "http://localhost:8088")
    )
    superset_username: str = field(
        default_factory=lambda: os.getenv("SUPERSET_USERNAME", "admin")
    )
    superset_password: str = field(
        default_factory=lambda: os.getenv("SUPERSET_PASSWORD", "admin")
    )
...
class TenantManager:
    ...
    async def create_tenant(
        ...
        job_type = (
            JobType.CLONE_DATABASE if clone_from_tenant_id else JobType.CREATE_DATABASE
        )
    ...
    def _row_to_tenant(self, row: asyncpg.Record) -> Tenant:
        ...
        return Tenant(
            ...
            parent_tenant_id=(
                str(row["parent_tenant_id"]) if row.get("parent_tenant_id") else None
            ),
            db_host=row["db_host"],
            db_port=row["db_port"],
            db_name=row["db_name"],
    ...
    async def _create_provisioning_job(
        self, tenant_id: str, job_type: JobType, config: Dict[str, Any]
    ) -> ProvisioningJob:
        ...
        job = ProvisioningJob(
            id=job_id, tenant_id=tenant_id, job_type=job_type, config=config
        )
    ...
    async def _execute_provisioning_job(self, job: ProvisioningJob) -> None:
        ...
        try:
            ...
            await self._update_job_status(
                job.id, JobStatus.RUNNING, "Starting provisioning"
            )
            ...
            # Mark complete
            await self._update_job_status(
                job.id, JobStatus.COMPLETED, "Provisioning complete"
            )
    ...
    async def _provision_new_database(self, job: ProvisioningJob) -> None:
        ...
        try:
            await self._update_job_status(
                job.id, JobStatus.RUNNING, "Creating database user", 10
            )
            ...
            await self._update_job_status(
                job.id, JobStatus.RUNNING, "Creating database from template", 30
            )
            ...
            await self._update_job_status(
                job.id, JobStatus.RUNNING, "Granting permissions", 70
            )
    ...
    async def _clone_database(self, job: ProvisioningJob) -> None:
        ...
        if source_tenant_id:
            ...
            if source_tenant:
                await self._update_job_status(
                    job.id, JobStatus.RUNNING, "Copying data", 80
                )
                # Use pg_dump/pg_restore for data copy
                # This is simplified - in production use subprocess with pg_dump
                logger.info(
                    f"Would copy data from {source_tenant.db_name} to {job.config['db_name']}"
                )

    async def _update_job_status(
        self, job_id: str, status: JobStatus, step: str, progress: int = 0
    ) -> None:
        ...
        async with self.pool.acquire() as conn:
            if status == JobStatus.RUNNING:
                ...
                """,
                    ...
                    step,
                    datetime.utcnow(),
                    step if status == JobStatus.FAILED else None,
                    job_id,
                )
...
class SchemaIntrospector:
    ...
    async def _get_tables(self, conn: asyncpg.Connection) -> List[TableSchema]:
        ...
        for table_row in table_rows:
            ...
            # Get primary key
            table.primary_key = await self._get_primary_key(
    ...
    async def generate_markdown(self) -> str:
        ...
        lines.append("")
        lines.append(
            f"*Generated: {schema.introspected_at.strftime('%Y-%m-%d %H:%M:%S UTC')}*"
        )
        lines.append("")
        ...
        for table in schema.tables:
            lines.append(
                f"- [{table.schema}.{table.name}](#{table.schema}{table.name})"
            )
        lines.append("")
        ...
        for table in schema.tables:
            ...
            for col in table.columns:
                ...
                lines.append(
                    f"| {name} | `{col_type}` | {nullable} | {default} | {comment} |"
                )
            ...
            if table.indexes:
                ...
                for idx in table.indexes:
                    if idx["index_name"] not in seen:
                        ...
                        lines.append(
                            f"- `{idx['index_name']}`: {idx['column_name']}{unique}{primary}"
                        )
...
class SupersetManager:
    ...
    def __init__(
        self, superset_url: str = None, username: str = None, password: str = None
    ):
    ...
    async def create_database_connection(
        self, tenant: Tenant, database_name: Optional[str] = None
    ) -> int:
        """
    ...
    async def create_datasets_from_schema(
        ...
        for table in schema.tables:
            ...
            if response.status_code == 201:
                ...
                logger.warning(
                    f"Failed to create dataset for {table_full_name}: {response.text}"
                )
    ...
    async def setup_tenant_workspace(self, tenant: Tenant) -> Dict[str, Any]:
        ...
        exclude = [
            "public.ir_model",
            "public.ir_model_fields",  # Odoo system
            "public.audit_log",  # Don't expose audit
        ]
        dataset_ids = await self.create_datasets_from_schema(
            db_id, schema, exclude_tables=exclude
        )
...
class DocumentationGenerator:
    ...
    async def generate_all(
        self, dsn: str, tenant_code: str, formats: Optional[List[str]] = None
    ) -> Dict[str, Path]:
        """
...
async def main():
    ...
    create_parser.add_argument(
        "--type", choices=["client", "provider"], default="client"
    )
    ...
    list_parser.add_argument(
        "--type", choices=["client", "provider", "all"], default="all"
    )
    list_parser.add_argument(
        "--status", choices=["active", "suspended", "all"], default="all"
    )

    # Introspect
    introspect_parser = subparsers.add_parser(
        "introspect", help="Introspect database schema"
    )
    introspect_parser.add_argument("tenant_code", help="Tenant code")
    introspect_parser.add_argument(
        "--format", choices=["dbml", "markdown", "json", "all"], default="all"
    )
    introspect_parser.add_argument(
        "--output", default="./docs", help="Output directory"
    )
    ...
    if args.command == "create":
        async with TenantManager() as manager:
            tenant_type = (
                TenantType.PROVIDER if args.type == "provider" else TenantType.CLIENT
            )
        ...
        async with TenantManager() as manager:
            ...
            for t in tenants:
                print(
                    f"{t.code:<20} {t.name:<30} {t.tenant_type.value:<12} {t.status.value:<12}"
                )
        ...
        async with TenantManager() as manager:
            ...
            formats = (
                ["dbml", "markdown", "json"] if args.format == "all" else [args.format]
            )
            generator = DocumentationGenerator(args.output)
            outputs = await generator.generate_all(
                tenant.db_connection_string, tenant.code, formats
            )
