...
def main(env):
    ...
    module_rows = [[m.name, m.state, m.latest_version, m.category_id.name or ""] for m in modules]
    export_csv("feature_inventory.modules.csv", ["module", "state", "version", "category"], module_rows)
    ...
    for p in projects:
        row = [
            ...
            p.allow_recurring_tasks if 'allow_recurring_tasks' in model_fields else "N/A",
            p.allow_task_dependencies if 'allow_task_dependencies' in model_fields else "N/A",
    ...
    export_csv("feature_flags.projects.csv", 
               ["project_name", "subtasks", "recurring", "deps", "milestones", "timesheets", "visibility"], 
               flag_rows)
    ...
    for p in projects:
        ...
        if backlog_stage_ids:
             backlog_count = env["project.task"].search_count(domain + [("stage_id", "in", backlog_stage_ids)])
        
        # Blocked count
        blocked_count = env["project.task"].search_count(domain + [("kanban_state", "=", "blocked")])
        
        # Overdue count
        overdue_count = env["project.task"].search_count(domain + [
            ("date_deadline", "<", datetime.now().date()), 
            ("state", "!=", "1_done") # Assuming state usage, or stage fold
        ])
        
        summary_rows.append([p.name, total, backlog_count, blocked_count, overdue_count])

    export_csv("backlog.summary.csv", 
               ["project_name", "total_tasks", "backlog_count", "blocked_count", "overdue_count"], 
               summary_rows)
