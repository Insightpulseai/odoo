...
| 3 | `platform` | Platform operations: monitoring, alerting, runbooks, SRE tooling (renamed from `ops-platform`) | `rg-ipai-shared-dev` (App Insights, WAF policies), `rg-ipai-devops` (ipai-devcenter, ipai-build-pool) | `mi-ipai-platform-dev` / `mi-ipai-platform-prod` | GitHub Actions | Must not own application container definitions; must not duplicate infra IaC |
...
| 10 | `design` | Shared UI components, tokens, and style guides (renamed from `design-system`) | N/A | N/A | GitHub Actions (npm publish) | Must not contain backend logic or Azure resource definitions |
...
| `ipai-plane-dev` | Container App | `platform` |
| `ipai-shelf-dev` | Container App | `platform` |
| `ipai-auth-dev` | Container App | `infra` (Keycloak config) |
| `ipai-mcp-dev` | Container App | `agents` |
| `ipai-ocr-dev` | Container App | `agents` |
| `ipai-superset-dev` | Container App | `data-intelligence` |
...
| App Insights | Monitoring | `platform` |
| WAF Policy | Security | `infra` |
| `mi-ipai-platform-dev` | Managed Identity | `platform` |
| `mi-ipai-lakehouse-dev` | Managed Identity | `data-intelligence` |
...
| `mi-ipai-platform-*` | Managed Identity | `platform` |
| `mi-ipai-odoo-*` | Managed Identity | `odoo` |
| `mi-ipai-lakehouse-*` | Managed Identity | `data-intelligence` |

### rg-ipai-ai-dev (AI and Data)

| Resource | Type | Owning Repo |
|----------|------|-------------|
| `dbw-ipai-dev` | Databricks Workspace | `data-intelligence` (execution surface) |
...
| `stipaidevlake` | Storage Account (Data Lake) | `data-intelligence` |
...
| Resource | Type | Owning Repo |
|----------|------|-------------|
| `pg-ipai-dev` | PostgreSQL | `infra` (provisioned), `platform` (consumed) |
...
| Resource | Type | Owning Repo |
|----------|------|-------------|
| `ipai-devcenter` | Dev Center | `platform` |
| `ipai-build-pool` | Build Pool | `platform` |

### rg-data-intel-ph

| Resource | Type | Owning Repo |
|----------|------|-------------|
| `data-intel-ph-resource` | AI Foundry | `data-intelligence` |
...
| Databricks workspace vs repo boundary | Notebooks may be edited in-workspace without Git sync | All notebooks, jobs, and pipelines authored in `data-intelligence` repo, deployed via Databricks Asset Bundles | Enforce Repos integration + CI deployment |
...
| Superset ownership | Superset container in `rg-ipai-dev` | Owning repo should be `data-intelligence` (analytics surface) | Update CI/CD pipeline ownership |
...
|------|--------------------|--------------------|
| `infra` | IaC -- Azure, ~~DigitalOcean~~ (deprecated), Cloudflare, Terraform, Bicep | IaC -- Azure-native platform, Cloudflare DNS/edge, Terraform, Bicep |
| `data-intelligence` | Intelligence / Analytics pipelines (Databricks, Medallion) — renamed from `lakehouse` | Databricks lakehouse: medallion pipelines, notebooks, jobs, SQL, and analytics deployment assets |
...
| `mcp-core` | MCP coordination moved to `agents` + `platform` | `agents` |
...
| `mi-ipai-lakehouse-dev` | `data-intelligence` | `rg-ipai-shared-dev` | Databricks, storage, Superset access |
| `mi-ipai-lakehouse-prod` | `data-intelligence` | `rg-ipai-shared-prod` | Production lakehouse workload identity |
| `mi-ipai-lakehouse-staging` | `data-intelligence` | `rg-ipai-shared-staging` | Staging lakehouse workload identity |
| `mi-ipai-platform-dev` | `platform` | `rg-ipai-shared-dev` | Platform tooling, monitoring, Dev Center |
| `mi-ipai-platform-prod` | `platform` | `rg-ipai-shared-prod` | Production platform workload identity |
| `mi-ipai-platform-staging` | `platform` | `rg-ipai-shared-staging` | Staging platform workload identity |
...
2. **Databricks rule**: The Databricks workspace (`dbw-ipai-dev`) is an execution surface. The `data-intelligence` repo (renamed from `lakehouse`) is the Git SSOT for all notebooks, jobs, SQL, and pipeline definitions. In-workspace editing without Git sync is prohibited.
