...
## Top-Level Directory Allowlist (Hard Gate)

The canonical allowlist is:  `ssot/repo/top_level_allowlist.yaml`
The enforcement script is:   `scripts/repo/enforce_top_level_allowlist.py`
The CI gate is:              `.github/workflows/repo-top-level-guard.yml`

**Rule:** Any PR that introduces a top-level directory not in the allowlist will **fail CI**.
To add a new directory: update the allowlist in the same PR, with `tier`, `purpose`, and `owner`.

### Canonical top-level boundaries (Tier 0/1)

New domain work **must** map into one of these existing roots instead of creating a new top-level dir:

| Root | Domain | Toolchain |
|------|--------|-----------|
| `infra/` | IaC, DNS, Cloudflare, DigitalOcean, Vercel, Terraform | Terraform, YAML, Python |
| `platform/` | Supabase platform kit, advisor, service registry | TS, YAML |
| `web/` | Next.js apps, control plane, AI control plane | TypeScript, Next.js |
| `agents/` | Agent registry, skills, workflows, Claude/Gemini agents | Markdown, YAML |
| `apps/` | Standalone applications (iOS, ops-console, OCR) | Swift, TypeScript |
| `docs/` | Architecture, runbooks, contracts, ops docs | Markdown |
| `spec/` | Spec Kit bundles per feature | Markdown |
| `scripts/` | Tooling, CI checks, DNS, health, release | Python, Bash |
| `supabase/` | DB schema, Edge Functions, RLS, seeds | SQL, TypeScript (Deno) |
| `addons/` | Odoo addons: CE, OCA, ipai | Python |
| `automations/` | n8n workflow JSON exports | JSON |
| `ssot/` | SSOT registries: secrets, repo, skills, env | YAML |
| `design/` | Design tokens, Figma exports | JSON |
| `packages/` | Shared pnpm packages (`@ipai/*`) | TypeScript |
| `templates/` | Reusable scaffolding templates | TypeScript, Python |
| `tests/` | Cross-module test suites | Python, TypeScript |
| `tools/` | Developer tools, audit, parity analysis | Python |

### `artifacts/` — generated outputs only

```
artifacts/                   ← GENERATED OUTPUTS. Never SSOT inputs.
├── dns/                     ← DNS state exports (generated by CI)
├── cloudflare/              ← Cloudflare zone snapshots (generated by drift check)
└── figma/                   ← Figma exports (generated by design pipeline)
```

**Invariant:** If you find yourself writing a config or SSOT file under `artifacts/`, stop.
Move it to the appropriate `infra/`, `platform/`, `docs/`, or `ssot/` subdirectory instead.
