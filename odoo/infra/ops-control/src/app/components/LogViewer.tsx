...
import { X, Download, Terminal, CheckCircle, AlertCircle, AlertTriangle, Info, ExternalLink, FileText, File } from "lucide-react";
...
export function LogViewer({ title, events, artifacts, onClose }: LogViewerProps) {
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-300">
                      ...
                      <div className="space-y-2">
                        {artifacts.map((artifact, idx) => {
                          const ArtifactIcon = artifact.kind === 'link' ? ExternalLink : artifact.kind === 'diff' ? FileText : File;
                          
                          return (
                            <div
                              key={idx}
                              className="p-3 rounded-lg bg-indigo-50 border border-indigo-200 animate-in fade-in slide-in-from-bottom-2"
                            >
                              <div className="flex items-center gap-3">
                                <ArtifactIcon className="h-4 w-4 text-indigo-600 flex-shrink-0" />
                                <div className="flex-1">
                                  <div className="font-semibold text-slate-900 text-xs">{artifact.title}</div>
                                  {artifact.kind === 'link' ? (
                                    <a
                                      href={artifact.value}
                                      target="_blank"
                                    ...
                                    <div className="text-xs text-slate-600 mt-1 font-mono max-h-32 overflow-y-auto">
                                      <pre className="whitespace-pre-wrap break-all">
                                        {artifact.value}
                                      </pre>
                                    </div>
