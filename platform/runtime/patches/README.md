...
|-------|-----------|-----------|---------------|
| `oca_mail_tracking_odoo18_prepare_outgoing_list.patch` | `addons/oca/mail/mail_tracking/models/mail_mail.py` | OCA rigid Odoo18 signature breaks Odoo18 MRO when `mass_mailing` passes `doc_to_followers=` | Odoo 18.0 / OCA mail 17.0 — 2026-02-27 |
| `oca_account_usability_odoo18_anglo_saxon.patch` | `addons/oca/account-financial-tools/account_usability/` | Odoo18 removed `anglo_saxon_accounting`; OCA re-add causes Owl `UncaughtPromiseError` | Odoo 18.0 / OCA account-financial-tools 17.0 — 2026-02-27 |
| `odoo-test-helper/0001-odoo18-metamodel-module_to_models.patch` | `odoo-test-helper` package | MetaModel refactor in Odoo18: `module_to_models` → `_module_to_models__` | Odoo 18.0 / odoo-test-helper 2.0.x — 2026-02-27 |
...
- Odoo minor version bump (18.0.x → 18.0.y)
...
### `oca_mail_tracking_odoo18_prepare_outgoing_list.patch`
...
positional args and a rigid signature that breaks with Odoo 18's MRO when
...
```bash
cd addons/oca/mail
git apply ../../../runtime/patches/oca_mail_tracking_odoo18_prepare_outgoing_list.patch
```
...
### `oca_account_usability_odoo18_anglo_saxon.patch`

**Applies to:** `addons/oca/account-financial-tools/`

**Why:** Odoo 18 removed the `anglo_saxon_accounting` field from `res.config.settings` and changed its inventory valuation strategy. The `account_usability` module attempts to add this field to the Accounting settings view, which causes an Owl `UncaughtPromiseError` and prevents the settings page from loading.
...
git apply ../../../runtime/patches/oca_account_usability_odoo18_anglo_saxon.patch
```

---

### `odoo-test-helper/0001-odoo18-metamodel-module_to_models.patch`

**Applies to:** `odoo-test-helper` package

**Why:** Renames `module_to_models` → `_module_to_models__` to match Odoo 18
