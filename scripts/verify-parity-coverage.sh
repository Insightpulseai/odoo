...
if [[ -f ".github/workflows/build-and-deploy.yml" ]]; then
    echo -e "${GREEN}✅ Git branch → environment mapping${NC}"
    ((PASSED_CHECKS++))
else
    echo -e "${RED}❌ Missing: .github/workflows/build-and-deploy.yml${NC}"
    ((FAILED_CHECKS++))
fi

# 1.2 Automatic builds on push
if grep -q "on: \[push\]" .github/workflows/*.yml 2>/dev/null; then
    echo -e "${GREEN}✅ Automatic builds on push${NC}"
    ((PASSED_CHECKS++))
else
    echo -e "${RED}❌ Missing: GitHub Actions push trigger${NC}"
    ((FAILED_CHECKS++))
fi
