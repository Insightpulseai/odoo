                ...
                if isinstance(node, ast.ClassDef):
                    ...
                    for base in node.bases:
                        if isinstance(base, ast.Attribute):
                            if base.attr == 'Model':
                                model_info = extract_model_details(node, model_file, content)
                                if model_info:
        ...
        if isinstance(item, ast.Assign):
            for target in item.targets:
                if isinstance(target, ast.Name):
                    ...
                    if target.id == '_description' and isinstance(item.value, ast.Constant):
                        description = item.value.value

                    # Extract fields (simplified detection)
                    if hasattr(item.value, 'func'):
                        if hasattr(item.value.func, 'attr'):
                            if item.value.func.attr in ['Many2one', 'One2many', 'Many2many', 'Char', 'Text', 'Integer', 'Float', 'Boolean', 'Date', 'Datetime', 'Selection']:
        ...
        try:
            ...
            for record in root.findall('.//record[@model="res.groups"]'):
                ...
                groups.append({
                    ...
                    'category': category.get('ref', '') if category is not None else '',
                    'file': str(xml_file)
                })
        except:
            pass
    ...
    for xml_file in views_dir.glob('*.xml'):
        try:
            ...
            for record in root.findall('.//record[@model="ir.ui.view"]'):
                ...
                deprecated_type_xml = arch.get('type') == 'xml' if arch is not None else False

                # Detect view type from arch content
                view_type = ''
                if arch is not None:
                    for child in arch:
                        if child.tag in ['tree', 'form', 'kanban', 'search', 'graph', 'pivot', 'calendar']:
                            view_type = child.tag
                            break
    ...
    for xml_file in data_dir.glob('*.xml'):
        try:
            ...
            for record in root.findall('.//record[@model="ir.cron"]'):
                ...
                numbercall = record.find('.//field[@name="numbercall"]')  # Deprecated in Odoo 18
                ...
                crons.append({
                    ...
                    'file': str(xml_file),
                    'deprecated_numbercall': numbercall is not None  # Flag Odoo 18 issue
                })
        except:
            pass
...
def generate_module_doc(module_name: str) -> str:
    ...
    for cron in cron_jobs:
        if cron.get('deprecated_numbercall'):
            odoo18_issues.append(f"⚠️ Deprecated `numbercall` field in cron job: `{cron['xml_id']}` ({cron['file']})")

    # Check for deprecated type="xml" in views
    for view in views:
        if view.get('deprecated_type_xml'):
            odoo18_issues.append(f"⚠️ Deprecated `type=\"xml\"` attribute in view: `{view['xml_id']}` ({view['file']})")
...
"""
    ...
    # Dependencies
    core_deps = [dep for dep in manifest.get('depends', []) if not dep.startswith('ipai_')]
    ...
    if models:
        ...
        for model in models:
            ...
            if model['fields']:
                ...
                doc += "**Fields**: No fields extracted (check source code manually)\n\n"
    else:
    ...
    if views:
        ...
        doc += "|---------|------|-------|------|------|\n"
        for view in views:
            deprecated_marker = " ⚠️ (deprecated type='xml')" if view.get('deprecated_type_xml') else ""
