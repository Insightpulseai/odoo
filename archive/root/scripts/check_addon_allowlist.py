...
def main():
    ...
    found_modules = [
        f.name for f in IPAI_ROOT.iterdir() if f.is_dir() and not f.name.startswith(".")
    ]

    for mod in found_modules:
        if mod == "scripts":
            continue  # Ignore scripts directory
    ...
    if errors:
        ...
        print(
            "Policy: All new EE replacement logic must go into 'ipai_enterprise_bridge'."
        )
