...
project_modules = IrModule.search(
    [("name", "like", "project%"), ("state", "=", "installed")]
)
...
task_fields = IrModelFields.search(
    [("model", "=", "project.task"), ("name", "in", task_fields_to_check)]
)
...
project_fields = IrModelFields.search(
    [("model", "=", "project.project"), ("name", "in", project_fields_to_check)]
)
...
task_type_fields = IrModelFields.search(
    [("model", "=", "project.task.type"), ("store", "=", True)]
)
...
# Build DBML content
dbml_lines = [
    "// Odoo 18 CE Project ERD - Auto-generated",
...
""".format(
    ...
    milestone_headers=(
        ",".join(milestone_headers)
        if model_existence.get("project.milestone")
        else "N/A - model not found"
    ),
...
print("\nMilestone:")
print(
    f"  - project.milestone model: {'FOUND' if model_existence.get('project.milestone') else 'NOT FOUND'}"
)
print(
    f"  - milestone_id on project.task: {'FOUND' if 'milestone_id' in task_field_info else 'NOT FOUND'}"
)
