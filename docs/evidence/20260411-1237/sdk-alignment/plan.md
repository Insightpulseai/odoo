...
## 23. Control Plane Implementation Model

### Goal
Implement Pulser as a SaaS platform with explicit service and tenant control planes.

### Repo placement
- **Executable code**: Control-plane logic (onboarding, rollout targeting) belongs in `agent-platform`.
- **Infrastructure**: Provisioning logic for stamps and RG-level RBAC belongs in `infra`.
- **Business objects**: Tenant-related entities with ERP relevance remain in `odoo`.

## 24. Deployment Stamp Topology

### Canonical model
Deployment stamps are the unit of resilience and scale.
- **Components**: Runtime services, agent services, observability, rollout health gates.
- **Expansion**: New regions or higher capacity requirements are addressed by introducing new stamps.

## 25. Safe Rollout and Shift-Right Model

### Progression
1. **Validation**: CI/CD pipeline stages (CI → Staging).
2. **Canary**: Limited production exposure (Revision + Label-based routing).
3. **Staged Promotion**: Stamp-by-stamp rollout.
4. **Monitoring**: Automated health gates and smoke tests.

### Shift-right
Execute fault-injection (chaos) and failover/resiliency testing in production-like environments to validate behavior under real traffic loads.

## 26. Live-Site Operations Model

### Posture
- **Engineering Responsibility**: The engineering team that builds the feature owns the reliability in production.
- **Actionable Telemetry**: Primary focus on customer-impacting signals rather than raw infrastructure metrics.
- **Hotfix discipline**: Priority hotfixes must still pass automated CI checks before being promoted through the emergency rollout lane.
