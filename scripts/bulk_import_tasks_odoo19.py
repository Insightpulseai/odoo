...
print(f"Project (Close): {PROJECT_CLOSE} [found]")
...
print(f"Project (Tax):   {PROJECT_TAX} [found]")
...
for code, emp in EMPLOYEES.items():
    ...
    if users:
        ...
        print(f"  User {code}: mapped to existing Odoo user (matched by login)")
    else:
        # Try by name
        users = _kw("res.users", "search", [[("name", "ilike", emp["name"])]], {"limit": 1})
        if users:
            user_map[code] = users[0]
            print(f"  User {code}: mapped to existing Odoo user (matched by name)")
        else:
            print(f"  User {code}: no matching Odoo user found [will skip assignment]")
