...
def convert():
    ...
    for t in tasks:
        ...
        if t["stage"] == "Preparation":
            ...
            schedules[key]["responsible_prep_id"] = t[
                "person_code"
            ]  # Need to handle Many2one mapping later, for now string/char or ignore
        elif t["stage"] == "Review":
    ...
    for (form, period), data in schedules.items():
        ...
        xml_content += (
            f'        <record id="{record_id}" model="ipai.bir.form.schedule">\n'
        )
        xml_content += (
            f'            <field name="form_code">{data["form_code"]}</field>\n'
        )
        xml_content += f'            <field name="period">{data["period"]}</field>\n'
        xml_content += (
            f'            <field name="bir_deadline">{data["bir_deadline"]}</field>\n'
        )

        if "prep_date" in data:
            xml_content += (
                f'            <field name="prep_date">{data["prep_date"]}</field>\n'
            )
        if "review_date" in data:
            xml_content += (
                f'            <field name="review_date">{data["review_date"]}</field>\n'
            )
        ...
        for i, step in enumerate(data["steps"]):
            ...
            xml_content += (
                f'                    <field name="title">{step["stage"]}</field>\n'
            )
            xml_content += (
                f'                    <field name="role">{step["role"]}</field>\n'
            )
