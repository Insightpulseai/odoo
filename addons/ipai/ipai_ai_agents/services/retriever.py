...
class KBRetriever:
    ...
    def __init__(
        self, supabase_url: str, supabase_key: str, rpc_embedding: str, rpc_text: str
    ):
    ...
    def from_env(cls) -> "KBRetriever":
        ...
        if not url or not key:
            _logger.warning(
                "Missing IPAI_SUPABASE_URL or key env vars - retrieval disabled"
            )
    ...
    def retrieve(
        self, tenant_ref: str, query: str, limit: int = 8
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _call_rpc(self, rpc_name: str, payload: Dict[str, Any]) -> Any:
        ...
        if not requests:
            raise RuntimeError(
                "Python package 'requests' is required for KB retrieval."
            )
        ...
        try:
            ...
            if resp.status_code >= 300:
                _logger.warning(
                    "Supabase RPC error %s: %s", resp.status_code, resp.text[:500]
                )
                return []
            return resp.json()
    ...
    def _maybe_embed(self, text: str) -> Optional[List[float]]:
        ...
        model = (
            os.environ.get("IPAI_EMBEDDINGS_MODEL") or "text-embedding-3-small"
        ).strip()
        ...
        try:
            ...
            if resp.status_code >= 300:
                _logger.warning(
                    "Embedding API error %s: %s", resp.status_code, resp.text[:300]
                )
                return None
            data = resp.json()
