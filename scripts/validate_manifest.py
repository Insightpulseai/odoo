...
def get_live_modules():
    ...
    try:
        ...
        if not uid:
            print(
                "⚠️  Authentication Failed: Could not login to Odoo. Using fallback list."
            )
...
def check_dependencies(manifest_path, data, valid_modules):
    ...
    for dep in depends:
        if dep not in valid_modules:
            print(
                f"   [Depends] ❌ Missing: Module '{module_name}' depends on '{dep}' (Not found in Local Repo or Live DB/Fallback)"
            )
            all_valid = False
    return all_valid
...
def check_icon(manifest_path, data):
    ...
    if module_name != os.path.basename(module_dir):
        print(
            f"   [Icon] ❌ Scope: Path starts with '{module_name}', expected '{os.path.basename(module_dir)}'"
        )
        return False
...
def main():
    ...
    for root, dirs, files in os.walk(ADDONS_DIR):
        if "__manifest__.py" in files:
            ...
            if (
                "node_modules" in manifest_path
                or "out/" in manifest_path
                or "site-packages" in manifest_path
            ):
                continue
            ...
            if not check_icon(manifest_path, data):
                has_error = True
            if not check_dependencies(manifest_path, data, valid_modules):
                has_error = True
