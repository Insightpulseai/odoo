...
**Audit Status:** ⚠️ v0.9.0 has 3 critical issues → **Deploy v0.9.1 instead**

## Executive Summary

**Current Image:** `ghcr.io/jgtolentino/odoo:v0.9.0` ❌ **NOT PRODUCTION-READY**
**Fixed Image:** `ghcr.io/jgtolentino/odoo:v0.9.1` ✅ **PRODUCTION-READY**

**Critical Fixes in v0.9.1:**
1. ✅ Python requirements auto-installation
2. ✅ Environment variable defaults (HOST, PORT, USER, PASSWORD, DB)
3. ✅ Health check configuration for Docker/Kubernetes
...
- DNS: `erp.insightpulseai.com` → 159.223.75.148 (A record)
...
3. Click: **Resize** → Select **8GB RAM / 4vCPU** ($48/month)
...
# Expected: ghcr.io/jgtolentino/odoo  v0.9.1  [IMAGE_ID]  [SIZE]

# Push to registry
docker push "$IMAGE"

# Verify push
echo "✅ Image available at: https://github.com/jgtolentino/odoo/pkgs/container/odoo"
```
...
sed -i 's|ghcr.io/jgtolentino/odoo:.*|ghcr.io/jgtolentino/odoo:v0.9.1|g' docker-compose.prod.yml

# Verify change
grep "image:" docker-compose.prod.yml
# Expected: image: ghcr.io/jgtolentino/odoo:v0.9.1
...
services:
  ...
  odoo:
    image: ghcr.io/jgtolentino/odoo:v0.9.1  # ← UPDATED
    container_name: odoo
    ...
    volumes:
      ...
      - odoo-logs:/var/log/odoo  # ← NEW (optional)
    ports:
      - "127.0.0.1:8069:8069"
...
# ✅ No critical errors
# ❌ If you see "CHANGE_ME_*" warnings, stop and fix .env.production
```

**Expected Logs (healthy deployment):**
```
odoo  | odoo.service.server: HTTP service (werkzeug) running on 0.0.0.0:8069
odoo  | odoo.modules.loading: Modules loaded.
odoo  | odoo.addons.base.models.ir_http: Generating routing map for key 1
```
...
if [ "$MODULE_COUNT" -eq 5 ]; then
    ...
    echo -e "${RED}❌ FAIL (expected 5, found $MODULE_COUNT)${NC}"; exit 1
fi

echo ""
echo -e "${GREEN}✅ All checks passed - System operational${NC}"
EOF
...
docker compose ps
# Expected: odoo STATUS = Up (healthy)
...
# Common causes:
# 1. Database password mismatch → Fix .env.production
# 2. Port 8069 already in use → Stop conflicting service
# 3. Out of memory → Verify VPS upgrade to 8GB
```
...
2. **Resource Usage:** `docker stats odoo` shows MEM < 2GB, CPU < 50%
3. **Web Access:** https://erp.insightpulseai.com loads without errors
...
- **GitHub:** https://github.com/jgtolentino/odoo
